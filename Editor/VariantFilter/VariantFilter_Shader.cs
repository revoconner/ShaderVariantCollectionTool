...
namespace ShaderVariantsCollectionTool
{
    ...
    class VariantFilter_ShaderEditor : ShaderVariantCollectionToolEditor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            foldoutValue = EditorGUILayout.Foldout(foldoutValue, "Add and delete exchange operation");
            ...
            if (foldoutValue)
            {
                ...
                mOperatorIndex1 = EditorGUILayout.IntField("Operation index 1", mOperatorIndex1);
                mOperatorIndex2 = EditorGUILayout.IntField("Operation index 2", mOperatorIndex2);
                EditorGUILayout.EndHorizontal();
                mOperatorShader = EditorGUILayout.ObjectField("Add Shader", mOperatorShader, typeof(Shader)) as Shader;

                var targetObj = target as VariantFilter_Shader;
                
                EditorGUILayout.BeginHorizontal();
                if (GUILayout.Button("Add to operation index 1"))
                {
                ...
                if (GUILayout.Button("Delete operation index 1"))
                {
                    if (mOperatorIndex1 >= 0 && mOperatorIndex1 < targetObj.mShaders.Length)
                    {
                ...
                if (GUILayout.Button("Exchange operation index"))
                {
                    if (mOperatorIndex1 >= 0 && mOperatorIndex1 < targetObj.mShaders.Length
